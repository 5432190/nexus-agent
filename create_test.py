...
test_code = '''\"\"\"Tests for StripeTool integration.\"\"\"
import pytest
...
async def test_stripe_charge_success(tmp_path):
    ...
    wallet = Wallet(backend=Ed25519Backend(key_path=str(key_path)))
...
Path('tests/test_stripe.py').write_text(test_code, encoding='utf-8')
print('✅ tests/test_stripe.py created')
